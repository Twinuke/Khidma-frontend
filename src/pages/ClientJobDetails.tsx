...
export default function ClientJobDetails() {
  ...
  if (loading) return <View style={styles.loaderContainer}><ActivityIndicator size="large" color="#2563EB" /></View>;
  if (!job) return <View style={styles.loaderContainer}><Text style={styles.errorText}>Job not found.</Text></View>;
      ...
      <ScrollView contentContainerStyle={{flexGrow: 1}} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.summarySection}>
          ...
          <View style={styles.tagsRow}>
             ...
             <Text style={styles.dateText}>Posted {new Date(job.createdAt).toLocaleDateString()}</Text>
          </View>
        ...
        {activeTab === 'overview' ? (
          <View style={styles.contentPadding}>
            ...
            <View style={styles.detailsBox}>
              <View style={styles.detailRow}>
                ...
                <Text style={styles.detailText}>{job.category || 'General'}</Text>
              </View>
              <View style={styles.detailRow}>
                <Ionicons name="earth-outline" size={20} color="#64748B"/>
                <Text style={styles.detailText}>{job.isRemote ? 'Remote Work' : 'On-Site'}</Text>
              </View>
              <View style={styles.detailRow}>
          ...
          <View style={styles.contentPadding}>
            {bids.length === 0 ? (
              ...
              bids.map((item: any) => {
                const isFriend = connectedUserIds.includes(item.freelancer?.userId);
                  ...
                  <View key={item.bidId} style={styles.freelancerCard}>
                    ...
                    <View style={styles.cardHeader}>
                      <View style={styles.avatarContainer}>
                        {item.freelancer?.profileImageUrl ? (
                          <Image source={{ uri: item.freelancer.profileImageUrl }} style={styles.avatarImage} />
                        ) : (
                          <Text style={styles.avatarText}>{item.freelancer?.fullName?.[0] || 'U'}</Text>
                        )}
                      </View>
                      <View style={styles.headerInfo}>
                        <Text style={styles.freelancerName}>{item.freelancer?.fullName || 'Unknown'}</Text>
                    ...
                    <View style={styles.cardBody}>
                      <View style={styles.metaRow}>
                        ...
                        <Text style={styles.metaText}>Delivers in {item.deliveryTimeDays} days</Text>
                      </View>
                    ...
                    <View style={styles.cardActions}>
                      ...
                      >
                        <Ionicons name="chatbubble-outline" size={20} color="#334155" />
                      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  jobTitle: { fontSize: 24, fontWeight: '800', color: '#0F172A', lineHeight: 32, marginBottom: 12 },
  tagsRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 20 },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6, marginRight: 12 },
  statusText: { fontSize: 12, fontWeight: '700' },
  dateText: { color: '#64748B', fontSize: 13 },
  
  statsGrid: { flexDirection: 'row', backgroundColor: '#F8FAFC', borderRadius: 12, padding: 16, justifyContent: 'space-between' },
  statBox: { flex: 1, alignItems: 'center' },
  statLabel: { fontSize: 12, color: '#64748B', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 },
  statValue: { fontSize: 18, fontWeight: '700', color: '#0F172A' },
  statSeparator: { width: 1, backgroundColor: '#E2E8F0', height: '80%' },

  tabContainer: { flexDirection: 'row', backgroundColor: '#FFF', paddingHorizontal: 20, borderBottomWidth: 1, borderColor: '#E2E8F0' },
  tab: { marginRight: 24, paddingVertical: 16, borderBottomWidth: 2, borderColor: 'transparent' },
  ...
  descriptionText: { fontSize: 15, lineHeight: 24, color: '#334155', marginBottom: 24 },
  
  detailsBox: { backgroundColor: '#FFF', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: '#E2E8F0', gap: 16 },
  ...
  avatarContainer: { width: 48, height: 48, borderRadius: 24, backgroundColor: '#DBEAFE', alignItems: 'center', justifyContent: 'center', marginRight: 12, overflow: 'hidden' },
  ...
  priceTag: { backgroundColor: '#F0F9FF', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  ...
  proposalText: { color: '#334155', fontSize: 14, lineHeight: 20, fontStyle: 'italic' },

  cardActions: { flexDirection: 'row', gap: 12, borderTopWidth: 1, borderColor: '#F1F5F9', paddingTop: 16, alignItems: 'center' },
  chatButton: { padding: 10, borderRadius: 8, backgroundColor: '#F1F5F9', justifyContent: 'center', alignItems: 'center' },
  
  connectButton: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#E2E8F0', borderRadius: 8, flex: 1, justifyContent: 'center' },
  connectText: { marginLeft: 6, fontWeight: '600', color: '#0F172A', fontSize: 13 },

  // ✅ New "Connected" Badge Style
  connectedBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#D1FAE5', borderRadius: 8, flex: 1, justifyContent: 'center' },
  connectedText: { marginLeft: 6, fontWeight: '600', color: '#059669', fontSize: 13 },

  hireButton: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 10, borderRadius: 8, backgroundColor: '#2563EB' },
  hireButtonText: { color: '#FFF', fontWeight: '600', fontSize: 14 },
  
  hiredBadge: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 10, backgroundColor: '#DCFCE7', borderRadius: 8, gap: 6 },
  hiredText: { color: '#16A34A', fontWeight: '700' },
  closedText: { flex: 1, textAlign: 'center', color: '#94A3B8', fontStyle: 'italic', paddingVertical: 10 },
});